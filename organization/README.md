# Проект: SQL-запросы к базе «Управление сотрудниками, проектами и задачами»

В данном репозитории содержатся SQL-скрипты для:

1. Создания и наполнения базы данных (схема таблиц: Departments, Roles, Employees, Projects, Tasks).
2. Решения трёх практических задач с использованием рекурсивных запросов (`WITH RECURSIVE`).
...
- **Employees.ManagerID** → (self-reference) → **Employees.EmployeeID** (один сотрудник может управлять другими).
- **Employees.DepartmentID** → **Departments.DepartmentID**.
- **Employees.RoleID** → **Roles.RoleID**.
- **Projects.DepartmentID** → **Departments.DepartmentID**.
- **Tasks.AssignedTo** → **Employees.EmployeeID**.
- **Tasks.ProjectID** → **Projects.ProjectID**.

## 1.1 Скрипты создания таблиц
...
## 1.2 Скрипт наполнения тестовыми данными
...
### Условие 
```text
Найти всех сотрудников, подчиняющихся Ивану Иванову (EmployeeID=1), включая всех «уровней» подчинённых. Для каждого сотрудника вывести:
EmployeeID, EmployeeName, ManagerID, DepartmentName, RoleName
Названия проектов (через запятую, если есть, иначе NULL)
Названия задач (через запятую, если есть, иначе NULL)
Сортировать по имени сотрудника. Использовать WITH RECURSIVE.
```
## 2.2 Задача 2
### Условие
```text
То же, что и задача 1, но дополнительно:
Вывести общее количество задач (TotalTasks) для каждого сотрудника.
Вывести общее число прямых подчинённых (TotalSubordinates), не включая их подчинённых.
Если у сотрудника нет проектов/задач, выводить NULL в столбцах соответствующих списков.
```
## 2.3 Задача 3
### Условие
```text
